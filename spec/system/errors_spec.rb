require 'spec_helper'

RSpec.describe 'errors', type: :system do
  it 'renders a page for missing' do
    visit missing_url

    expect(page).to have_content('The page you were looking for does not exist.')
    expect(page).to have_content('You may have mistyped the address or the page may have moved.')
    expect(page).to have_content('If you continue to experience problems please contact us at info@ksylvest.com.')
  end

  it 'renders a page for unprocessable' do
    visit unprocessable_url

    expect(page).to have_content('The change you wanted was rejected.')
    expect(page).to have_content('Maybe you tried to change something you do not have access to.')
    expect(page).to have_content('If you continue to experience problems please contact us at info@ksylvest.com.')
  end

  it 'renders a page for exception' do
    visit exception_url

    expect(page).to have_content('Something went wrong.')
    expect(page).to have_content('We have been notified of the issue and will try and fix it.')
    expect(page).to have_content('If you continue to experience problems please contact us at info@ksylvest.com.')
  end
end
